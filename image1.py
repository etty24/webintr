...
def main():
    ...
    if unggah_gambar is not None:
        ...
        #untuk membuat 2 kolom yang lebar sama
        kolom1, kolomtengah, kolom2 = st.columns(3)
        pilih_filter = kolomtengah.selectbox("pilih jenis filter:", ["BLUR","CONTOUR","SHARPEN","EMBOSS","SMOOTH"])

        kolom1.image(gambar, caption="GambarAsli",width=200)
        filtered_image = buat_filter(gambar, pilih_filter.upper())
        kolom2.image(filtered_image)
